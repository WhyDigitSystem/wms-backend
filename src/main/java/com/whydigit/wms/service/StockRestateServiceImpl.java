...
public class StockRestateServiceImpl implements StockRestateService {
	...
	public Map<String, Object> createStockRestate(StockRestateDTO stockRestateDTO) throws ApplicationException {
		...
		if (stockRestateDTO.getEntryNo() != null && stockRestateRepo.existsByEntryNoAndOrgIdAndClient(
				stockRestateDTO.getEntryNo(), stockRestateDTO.getOrgId(), stockRestateDTO.getClient())) {
		...
		for (StockRestateDetailsVO restateDetailsVO : restateDetailsVOs) {
			...
			if ("H".equals(restateVO.getTransferToFlag()) || "D".equals(restateVO.getTransferToFlag())) {

				NotificationVO notificationVO = new NotificationVO();
				notificationVO.setOrgId(restateVO.getOrgId());
				notificationVO.setPartno(restateDetailsVO.getPartNo());
				notificationVO.setPartDesc(restateDetailsVO.getPartDesc());
				notificationVO.setSku(restateDetailsVO.getSku());
				notificationVO.setClient(restateVO.getClient());
				notificationVO.setBranchCode(restateVO.getBranchCode());
				notificationVO.setWarehouse(restateVO.getWarehouse());
				notificationVO.setNotificationType("HoldAndDamage");
				notificationVO.setCreatedBy(restateVO.getCreatedBy());
				notificationVO.setUpdatedBy(restateVO.getCreatedBy());
				notificationVO.setSQty(restateDetailsVO.getToQty());
				notificationVO.setStatus(restateVO.getTransferToFlag());
				notificationVO.setGrnNo(restateDetailsVO.getGrnNo());
				notificationVO.setGrnDate(restateDetailsVO.getGrnDate());
				notificationVO.setBatch(restateDetailsVO.getBatch());
				notificationVO.setBatchDate(restateDetailsVO.getBatchDate());
				notificationVO.setBin(restateDetailsVO.getToBin());
				notificationVO.setExpDate(restateDetailsVO.getExpDate());
	...
	private boolean isHeaderValid(Row headerRow) throws ApplicationException {
		...
		List<String> expectedHeaders = Arrays.asList("Type", "From location", "From location type", "Location pick",
				"partno", "partdesc", "sku", "Grn No", "GRN date", "Batch No", "Exp date", "Entry no", "From Status",
				"To Status");
	...
	public String stockFreeze(Long orgid, String branch, String branchCode, String client, String freezeStatus) {
		...
		List<StockDetailsVO> stockDetailsVOList = stockDetailsRepo.findByOrgIdAndBranchAndBranchCodeAndClient(orgid,
				branch, branchCode, client);
