...
public interface CodeConversionRepo extends JpaRepository<CodeConversionVO, Long>{
	...
	@Query(nativeQuery = true,value ="select a.batch,a.batchdate,a.expdate,a.lotno from( SELECT partno,partdesc,sku,grnno,grndate,batch,batchdate,expdate,bintype,binclass,celltype,core,bin,status,qcflag,SUM(sqty) AS total_qty FROM stockdetails WHERE orgid =?1 AND branchcode =?2 AND client =?3 AND warehouse =?4 AND partno=?5 and grnno=?6 and bintype=?7 GROUP BY partno,partdesc,sku,grnno,grndate,batch,batchdate,expdate,bintype,status,binclass,celltype,core,bin,qcflag HAVING SUM(sqty) > 0)a group by a.batch,a.batchdate,a.expdate,a.lotno")
