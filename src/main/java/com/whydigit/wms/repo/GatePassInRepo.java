...
public interface GatePassInRepo extends JpaRepository<GatePassInVO, Long> {
	...
	@Query(nativeQuery =true,value = "SELECT \r\n"
			...
			+ "			  AND b.client = ?4\r\n"
			+ "			  AND b.warehouse = ?5\r\n"
			+ "			  AND (MONTH(b.docdate) = ?6 OR ?6 IS NULL)  AND (\r\n"
			+ "      ( ?7 = 'today'      AND DATE(b.docdate) = CURDATE())\r\n"
			+ "   OR ( ?7 = 'yesterday'  AND DATE(b.docdate) = CURDATE() - INTERVAL 1 DAY)\r\n"
			+ "   OR ( ?7 = 'this week'  AND DATE(b.docdate) BETWEEN\r\n"
			+ "            DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)\r\n"
			+ "            AND DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 6 DAY))\r\n"
			+ "   OR ( ?7 = 'this month' AND YEAR(b.docdate) = YEAR(CURDATE())\r\n"
			+ "                             AND MONTH(b.docdate) = MONTH(CURDATE()))\r\n"
			+ ")\r\n"
			...
			+ "			  AND a.warehouse = ?5\r\n"
			+ "			  AND a.client = ?4\r\n"
			+ "			  AND (MONTH(a.entrydate) = null OR null IS NULL)\r\n"
			+ "             AND (\r\n"
			+ "      ( ?7 = 'today'      AND DATE(a.entrydate) = CURDATE())\r\n"
			+ "   OR ( ?7  = 'yesterday'  AND DATE(a.entrydate) = CURDATE() - INTERVAL 1 DAY)\r\n"
			+ "   OR ( ?7 = 'this week'  AND DATE(a.entrydate) BETWEEN\r\n"
			+ "            DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)\r\n"
			+ "            AND DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 6 DAY))\r\n"
			+ "   OR ( ?7 = 'this month' AND YEAR(a.entrydate) = YEAR(CURDATE())\r\n"
			+ "                             AND MONTH(a.entrydate) = MONTH(CURDATE()))\r\n"
			+ ")\r\n"
	...
	Set<Object[]> getGrnDetails(Long orgId, String finYear, String branchCode, String client, String warehouse, String month,String type);
