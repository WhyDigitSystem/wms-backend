...
public interface BoExcelUploadRepo extends JpaRepository<BoExcelUploadVO, Long>{
	...
	@Query(value ="SELECT b.orderno, \r\n"
			...
			+ "             AND (\r\n"
			+ "      ( ?7 = 'today'      AND DATE(a.orderdate) = CURDATE())\r\n"
			+ "   OR ( ?7 = 'yesterday'  AND DATE(a.orderdate) = CURDATE() - INTERVAL 1 DAY)\r\n"
			+ "   OR ( ?7 = 'this week'  AND DATE(a.orderdate) BETWEEN\r\n"
			+ "            DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)\r\n"
			+ "            AND DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 6 DAY))\r\n"
			+ "   OR ( ?7 = 'this month' AND YEAR(a.orderdate) = YEAR(CURDATE())\r\n"
			+ "                             AND MONTH(a.orderdate) = MONTH(CURDATE()))\r\n"
			...
			+ "			        AND b.branchcode = ?2\r\n"
			+ "			        AND b.client = ?4\r\n"
			+ "			  ) GROUP BY a.orderno, a.orderdate",nativeQuery =true)
