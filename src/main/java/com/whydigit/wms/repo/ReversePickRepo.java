...
public interface ReversePickRepo extends JpaRepository<ReversePickVO, Long>{
	...
	@Query(nativeQuery =true,value ="SELECT * FROM reversepick where orgid=?1 and client=?2 and branch=?3 and branchcode=?4 and finyear=?5 and warehouse=?6 order by docid desc")
