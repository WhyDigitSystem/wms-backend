...
public interface PutAwayRepo extends JpaRepository<PutAwayVO, Long> {
	...
	@Query(value = "select a.docid,a.grnno, a.totalputawayqty,'Complete'status from putaway a where a.orgid =?1\r\n"
			...
			+ "              AND (\r\n"
			+ "      ( ?6  = 'today'      AND DATE(a.docdate) = CURDATE())\r\n"
			+ "   OR ( ?6 = 'yesterday'  AND DATE(a.docdate) = CURDATE() - INTERVAL 1 DAY)\r\n"
			+ "   OR ( ?6 = 'this week'  AND DATE(a.docdate) BETWEEN\r\n"
			+ "            DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)\r\n"
			+ "            AND DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 6 DAY))\r\n"
			+ "   OR ( ?6 = 'this month' AND YEAR(a.docdate) = YEAR(CURDATE())\r\n"
			+ "                             AND MONTH(a.docdate) = MONTH(CURDATE()))\r\n"
			+ ")\r\n"
			...
			+ "			  AND a.finyear =?2  and (month(a.docdate)= ?5 or ?5 is null)AND (\r\n"
			+ "      ( ?6 = 'today'      AND DATE(a.docdate) = CURDATE())\r\n"
			+ "   OR ( ?6 = 'yesterday'  AND DATE(a.docdate) = CURDATE() - INTERVAL 1 DAY)\r\n"
			+ "   OR ( ?6 = 'this week'  AND DATE(a.docdate) BETWEEN\r\n"
			+ "            DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)\r\n"
			...
			+ ")\r\n"
			+ "			  AND a.branchcode =?3\r\n"
			+ "			  AND a.client =?4  and a.docid\r\n"
			+ "              not in(select a.grnno from putaway a where a.orgid =?1 \r\n"
			+ "			  AND a.finyear =?2 \r\n"
			+ "			  AND a.branchcode =?3\r\n"
			+ "			  AND a.client =?4 group by a.grnno)",
    nativeQuery = true)
	Set<Object[]> getPutaway(Long orgId, String finYear, String branchCode, String client, String month,String type);
