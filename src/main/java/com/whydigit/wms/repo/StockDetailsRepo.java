...
@Query(nativeQuery =true,value ="select sum(sqty) from stockdetails  WHERE orgid = ?1 AND client = ?3 AND branchcode = ?2 AND warehouse = ?5 AND partno =?4 and grnno=?6 and bin='BULK' AND status = 'V';")
