...
@Query(nativeQuery = true,value = "select partno,partdesc,sku,grnno,grndate,batch,batchdate,expdate,bintype,binclass,celltype,core,bin,case when ?6='D' then 'Defective' else bin end toBin,case when ?6='D' then 'Defective' else bintype end toBinType,binclass tobinclass,celltype tocellType,core tocore, qcflag,sum(sqty) from stockdetails \r\n"
		+ "where orgid=?1 and branchcode=?2 and warehouse=?3 and client=?4 and status=?5\r\n"
