...
public interface CustomerRepo extends JpaRepository<CustomerVO, Long> {
	...
	@Query(nativeQuery = true,value ="select a.* from customer a ,clientbranch b,user_clientaccess c,users d where a.id=b.customerid  and a.customer=c.customer and d.users_id=c.users_id and a.org_id=?1 and d.user_name=?2   and b.branchcode=?3")
	List<CustomerVO> findAllAccessCustomerByUserName(Long orgid, String userName, String branchcode);
