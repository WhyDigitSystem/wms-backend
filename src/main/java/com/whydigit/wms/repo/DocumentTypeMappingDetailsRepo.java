...
public interface DocumentTypeMappingDetailsRepo extends JpaRepository<DocumentTypeMappingDetailsVO, Long> {
	...
	@Query(value = "select a from DocumentTypeMappingDetailsVO a where a.branch=?1 and a.client=?2 and a.finYear=?3 and a.screenCode=?4")
	DocumentTypeMappingDetailsVO findByBranchAndClientAndFinYearAndScreenCode(String branch, String client,
			String finYear, String screenCode);
